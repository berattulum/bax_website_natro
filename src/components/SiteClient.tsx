  ...
  return <>
    <header className="main-header">
      <div className="container header-container">
        ...
        <nav className={`main-nav${menuOpen ? ' is-open' : ''}`} aria-label="Main navigation"><ul>
          {[[copy.home, 'home'], [copy.about, 'about'], [copy.expertise, 'expertise'], [copy.references, 'references'], [copy.memberships, 'memberships'], [copy.contact, 'contact']].map(([label, id]) => <li key={id}><a href={`#${id}`} className={activeSection === id ? 'active' : undefined} aria-current={activeSection === id ? 'page' : undefined} onClick={() => { setActiveSection(id); setMenuOpen(false) }}>{label}</a></li>)}
        </ul></nav>
    ...
    <section id="home" className="hero-section"><div className="hero-slider">
      <div className="slide opening-slide active" aria-hidden="false">
        {slide === 0 ? <video autoPlay loop muted playsInline className="hero-video"><source src="/ucak-video.mp4" type="video/mp4" /></video> : <div className={`slide-bg ${active[3] || ''}`} />}
    ...
    <section id="about" className="about-section"><div className="container"><div className="about-flex"><div className="about-text"><h2><Heading text={d.aboutTitle || ''} /></h2></div><div className="about-desc"><p>{d.aboutDescription}</p><p>{d.aboutGoal}</p></div></div></div></section>
    <NarrativeScene scene={narrativeScenes[lang][0]} />
    <ExpertiseSection items={content.expertise} />
    <NarrativeScene scene={narrativeScenes[lang][1]} />
    <section className="process-section" aria-labelledby="process-title"><div className="container"><div className="process-heading"><span className="section-label">{copy.processLabel}</span><h2 id="process-title"><Heading text={d.processTitle || ''} /></h2></div><ol className="process-track">{copy.process.map(([title, text]) => <li key={title}><h3>{title}</h3><p>{text}</p></li>)}</ol></div></section>
