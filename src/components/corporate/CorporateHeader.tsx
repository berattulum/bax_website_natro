...
export function CorporateHeader({
  ...
  const selectLanguage = (nextLang: CorporateLang) => {
    ...
    document.cookie = `bax-locale=${nextLang}; Path=/; Max-Age=31536000; SameSite=Lax`
  ...
  return (
    <header className="main-header">
      <div className="container header-container">
        <div className="logo"><Link href={homeHref} aria-label="BaX Composites">
        ...
        <button type="button" className="mobile-menu-toggle" aria-expanded={menuOpen} aria-label={labels.menu} onClick={() => setMenuOpen((open) => { if (open) setOpenMegaMenu(null); return !open })}>
          <span /><span />
        </button>
        ...
        <nav className={`main-nav${menuOpen ? ' is-open' : ''}`} aria-label={lang === 'tr' ? 'Kurumsal navigasyon' : 'Corporate navigation'} onMouseEnter={() => megaCloseTimer.current && clearTimeout(megaCloseTimer.current)} onMouseLeave={scheduleMegaClose}><ul>
          <li className={`nav-with-submenu${openMegaMenu === 'about' ? ' is-submenu-open' : ''}`} onMouseEnter={() => showMegaMenu('about')}><div className="nav-parent-row"><Link href={hrefFor('company', lang)} className={active === 'profile' || active === 'founder' || active === 'records' ? 'active' : undefined}>{labels.about}</Link><button type="button" className="nav-submenu-toggle" aria-expanded={openMegaMenu === 'about'} aria-label={labels.about} onClick={() => toggleMegaMenu('about')}><span aria-hidden="true">⌄</span></button></div><div className="nav-submenu" aria-hidden={openMegaMenu !== 'about'}><Link href={hrefFor('company', lang)}>{labels.profile}</Link><Link href={hrefFor('founder', lang)}>{labels.founder}</Link><Link href={hrefFor('corporate', lang)}>{labels.records}</Link></div></li>
          <li className={`nav-with-submenu${openMegaMenu === 'expertise' ? ' is-submenu-open' : ''}`} onMouseEnter={() => showMegaMenu('expertise')}><div className="nav-parent-row"><Link className={active === 'capabilities' ? 'active' : undefined} href={hrefFor('capabilities', lang)}>{labels.capabilities}</Link><button type="button" className="nav-submenu-toggle" aria-expanded={openMegaMenu === 'expertise'} aria-label={labels.capabilities} onClick={() => toggleMegaMenu('expertise')}><span aria-hidden="true">⌄</span></button></div><div className="nav-submenu" aria-hidden={openMegaMenu !== 'expertise'}><Link href={hrefFor('capabilities', lang)}>{lang === 'tr' ? 'Tüm yetkinlikler' : 'Explore capabilities'}</Link></div></li>
          <li className={`nav-with-submenu${openMegaMenu === 'ecosystem' ? ' is-submenu-open' : ''}`} onMouseEnter={() => showMegaMenu('ecosystem')}><div className="nav-parent-row"><Link href={hrefFor('partnerships', lang)} className={active === 'partnerships' || active === 'networks' ? 'active' : undefined}>{labels.ecosystem}</Link><button type="button" className="nav-submenu-toggle" aria-expanded={openMegaMenu === 'ecosystem'} aria-label={labels.ecosystemMenu} onClick={() => toggleMegaMenu('ecosystem')}><span aria-hidden="true">⌄</span></button></div><div className="nav-submenu" aria-hidden={openMegaMenu !== 'ecosystem'}><Link href={hrefFor('partnerships', lang)}>{labels.partnerships}</Link><Link href={hrefFor('networks', lang)}>{labels.networks}</Link></div></li>
          <li className={`nav-with-submenu nav-sustainability${openMegaMenu === 'sustainability' ? ' is-submenu-open' : ''}`} onMouseEnter={() => showMegaMenu('sustainability')}><div className="nav-parent-row"><Link className={active === 'sustainability' ? 'active' : undefined} href={hrefFor('sustainability', lang)}>{labels.sustainability}</Link><button type="button" className="nav-submenu-toggle" aria-expanded={openMegaMenu === 'sustainability'} aria-label={labels.sustainability} onClick={() => toggleMegaMenu('sustainability')}><span aria-hidden="true">⌄</span></button></div><div className="nav-submenu" aria-hidden={openMegaMenu !== 'sustainability'}><Link href={hrefFor('sustainability', lang)}>{lang === 'tr' ? 'Yaklaşımımız' : 'Our approach'}</Link></div></li>
          ...
          <section className={openMegaMenu === 'about' ? 'is-current' : undefined}><span>{labels.about.toUpperCase()}</span><h3>{lang === 'tr' ? 'BaX’ı tanıyın' : 'Discover BaX'}</h3><Link href={hrefFor('company', lang)}>{labels.profile}</Link><Link href={hrefFor('founder', lang)}>{labels.founder}</Link><Link href={hrefFor('corporate', lang)}>{labels.records}</Link></section>
          <section className={openMegaMenu === 'expertise' ? 'is-current' : undefined}><span>{labels.capabilities.toUpperCase()}</span><h3>{lang === 'tr' ? 'Mühendislik' : 'Engineering'}</h3><Link href={hrefFor('capabilities', lang)}>{lang === 'tr' ? 'Tüm yetkinlikler' : 'Explore capabilities'}</Link></section>
          <section className={openMegaMenu === 'ecosystem' ? 'is-current' : undefined}><span>{labels.ecosystem.toUpperCase()}</span><h3>{lang === 'tr' ? 'Birlikte değer üretmek' : 'Creating value together'}</h3><Link href={hrefFor('partnerships', lang)}>{labels.partnerships}</Link><Link href={hrefFor('networks', lang)}>{labels.networks}</Link></section>
          <section className={openMegaMenu === 'sustainability' ? 'is-current' : undefined}><span>{labels.sustainability.toUpperCase()}</span><h3>{lang === 'tr' ? 'Döngüsel mühendislik' : 'Circular engineering'}</h3><Link href={hrefFor('sustainability', lang)}>{lang === 'tr' ? 'Yaklaşımımız' : 'Our approach'}</Link></section>
        </div></div></div></nav>

        <div className="header-right">
          <Link className={`header-contact-link${active === 'contact' ? ' is-active' : ''}`} href={hrefFor('contact', lang)}>{labels.contact}</Link>
