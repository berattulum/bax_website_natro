    ...
    <header className="main-header">
      <div className="container header-container">
        ...
        <nav className={`main-nav${menuOpen ? ' is-open' : ''}`} aria-label={lang === 'tr' ? 'Kurumsal navigasyon' : 'Corporate navigation'} onMouseEnter={() => megaCloseTimer.current && clearTimeout(megaCloseTimer.current)} onMouseLeave={scheduleMegaClose}><ul>
          ...
          <section className={openMegaMenu === 'about' ? 'is-current' : undefined}><span>{labels.about.toUpperCase()}</span><h3>{lang === 'tr' ? 'BaX’ı tanıyın' : 'Discover BaX'}</h3><Link href="/sirket-profili">{labels.profile}</Link><Link href="/kurucu">{labels.founder}</Link><Link href="/kurumsal-bilgiler">{labels.records}</Link></section>
          <section className={openMegaMenu === 'expertise' ? 'is-current' : undefined}><span>{labels.capabilities.toUpperCase()}</span><h3>{lang === 'tr' ? 'Mühendislik' : 'Engineering'}</h3><Link href="/capabilities">{lang === 'tr' ? 'Tüm yetkinlikler' : 'Explore capabilities'}</Link></section>
          <section className={openMegaMenu === 'ecosystem' ? 'is-current' : undefined}><span>{labels.ecosystem.toUpperCase()}</span><h3>{lang === 'tr' ? 'Birlikte değer üretmek' : 'Creating value together'}</h3><Link href="/is-ortakliklari">{labels.partnerships}</Link><Link href="/aglar-ve-uyelikler">{labels.networks}</Link></section>
          <section className={openMegaMenu === 'sustainability' ? 'is-current' : undefined}><span>{labels.sustainability.toUpperCase()}</span><h3>{lang === 'tr' ? 'Döngüsel mühendislik' : 'Circular engineering'}</h3><Link href="/surdurulebilirlik">{lang === 'tr' ? 'Yaklaşımımız' : 'Our approach'}</Link></section>
        </div></div></div></nav>
